...
# Load response texts from json file
with open("./jsons/texts.json", encoding="utf-8") as f:
    texts = json.load(f)

# Load custom dns list from json file
with open("./jsons/custom_dns.json", encoding="utf-8") as f:
    dns_list = json.load(f)["dns_options"]
...
async def show_all_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.from_user.id != bot_config["owner_id"]:
        return
    ...
    if args:
        ...
        user = get_all_user_info(user_id)
        if user:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dns_"):
        # DNS selected
        dns_value = data.split("dns_")[1].replace("_", ".")
        context.user_data["dns"] = dns_value
        server_id = context.user_data.get("server_id")

        creds = get_user(query.from_user.id)
        if not creds:
        ...
        try:
            ...
            await context.bot.send_message(chat_id=bot_config["owner_id"], text=msg, parse_mode="HTML")
        except Exception as e:
    ...
    keyboard = [
        [InlineKeyboardButton(d["name"], callback_data=f"dns_{d['value'].replace('.', '_')}")]
        for d in dns_list
    ]
